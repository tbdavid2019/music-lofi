...
### 🐛 Bug 修復
#### 雨聲 Loop 銜接頓挫修復
- ✅ 增加 GrainPlayer 的 `grainSize`（`1.2` → `2.0`）與 `overlap`（`0.4` → `0.8`）
- ✅ 消除了雨聲在循環銜接時的明顯斷裂感，現在完全無縫

### 🎵 音樂引擎優化
#### Cafe 風格全面重製
- ✅ **速度調整**：BPM 從 `84` 降至 `76`，更貼近真實咖啡廳的慵懶節奏。
- ✅ **和弦刷奏改善**：縮緊琶音間距（避免原本 `0.74` 秒的不自然間隔），模擬真實吉他手的觸弦。
- ✅ **三和弦優先**：將三和弦機率從 60% 提高至 75%，減少刺耳的七和弦衝突。
- ✅ **旋律更柔和**：降低旋律密度與力度，音量範圍從 `0.2-0.35` 降至 `0.15-0.28`。
- ✅ **幾乎無鼓**：大幅降低鼓組出現機率，營造純粹的鋼琴咖啡氛圍。
- ✅ **和聲配置改善**：重寫 `generateVoicing()` 演算法，使用開放配置（Open Voicing）避免音符堆疊。
- ✅ **全域音色柔化**：低通濾波器從 `2000Hz` 降至 `1400Hz`（更溫暖），增加混響深度。
